...
private:
	...
	/// <summary>
	/// Функция рекурсивно создаёт узел с заданным ключём в заданном поддереве, возвращаясь назад вызывает
	...
	/// <summary>
	/// Функция рекурсивно удаляет любой красный узел или черный узел с одним ребёнком в заданном поддереве
	/// </summary>
	/// <param name="node">корень поддерева с которого будет осуществляется поиск узла с заданным ключом</param>
	...
	/// <summary>
	/// Функция вызывается во время выхода из рекурсии для каждого узла в обратном порядке
	...
	/// <summary>
	/// Функция рекурсивно ищет узел с заданным ключём в заданном корне поддерева.
	/// </summary>
	/// <param name="node">корень поддерева.</param>
	/// <param name="key">ключ по которому осуществяется поиск элемента</param>
	/// <returns>
	/// искомы элемент или nullptr, если не найден элемент
	/// </returns>
	...
	/// <summary>
	/// Функция изменения цвета корня заданного узла и его детей на противоположный
	/// </summary>
	void FlipColor(RBTreeNode* node);

	/// <summary>
	/// Функция левого/правого поворот вокруг заданного узла в заданном направлении. Его левое/правое поддерево поднимается вверх, заменяя свой левый/правый дочерний узел
